...
function has_column(db: Database.Database, table: string, column: string): boolean {
  const cols = db.pragma(`table_info(${table})`) as { name: string }[];
  return cols.some((c) => c.name === column);
}

function has_table(db: Database.Database, table: string): boolean {
...
CREATE TABLE IF NOT EXISTS hazo_feedback_submissions_is_public_migration AS
  SELECT 1 WHERE 0;
...
export async function POST(): Promise<NextResponse> {
  try {
    ...
    // Run base schemas (all use IF NOT EXISTS — safe to re-run)
    db.exec(BASE_SCHEMA);
    db.exec(AUTH_SCHEMA);
    db.exec(NOTIFY_SCHEMA);

    // ── v2.1 migrations: events body columns ──────────────────────────────────
    if (!has_column(db, 'hazo_feedback_events', 'body_html')) {
      db.exec(`ALTER TABLE hazo_feedback_events ADD COLUMN body_html TEXT NULL`);
    }
    if (!has_column(db, 'hazo_feedback_events', 'body_text')) {
      db.exec(`ALTER TABLE hazo_feedback_events ADD COLUMN body_text TEXT NULL`);
    }

    // ── v2.1 migration: attachments — add event_id (legacy table without CHECK) ─
    // If the table was created by the old schema (no event_id column), recreate it.
    if (!has_column(db, 'hazo_feedback_attachments', 'event_id')) {
      db.exec(`
        CREATE TABLE hazo_feedback_attachments_v2 (
          id             TEXT PRIMARY KEY,
          submission_id  TEXT NULL,
          event_id       TEXT NULL,
          inline_id      TEXT NULL,
        ...
        INSERT INTO hazo_feedback_attachments_v2
        ...
        ALTER TABLE hazo_feedback_attachments_v2 RENAME TO hazo_feedback_attachments;
        CREATE INDEX IF NOT EXISTS idx_hf_attachments_submission
          ON hazo_feedback_attachments (submission_id);
        CREATE INDEX IF NOT EXISTS idx_hf_attachments_event
          ON hazo_feedback_attachments (event_id);
    ...
    // ── v2.1 migration: submissions — add is_public ───────────────────────────
    if (!has_column(db, 'hazo_feedback_submissions', 'is_public')) {
      db.exec(`ALTER TABLE hazo_feedback_submissions ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0`);
      db.exec(`
