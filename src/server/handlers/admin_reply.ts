...
export async function handle_admin_reply(
  ...
  const { getHazoConnect, getFileManager, adminScope, threadUrlBuilder, notifyOptions, logger } = opts;
  ...
  try {
    ...
    for (const entry of limited) {
      ...
      const remote_path = `feedback/${opts.appId}/${submission.id}/events/${event_id}/${attachment_id}`;
      ...
      if (!upload.success || !upload.data) {
        logger?.error('handle_admin_reply: file upload failed', { error: upload.error });
